...
vi.mock('@/lib/csrf', () => ({ csrfGuard: vi.fn().mockReturnValue(undefined) }));
...
vi.mock('@/lib/paymob', () => ({
  authenticate: vi.fn().mockResolvedValue('paymob-token'),
...
describe('payment initiation', () => {
  it('succeeds for valid pending order', async () => {
    ...
    const res = await POST(makeReq({ orderId: 'order-1', customer: { name: 'A', phone: '01000000000' } }));
  ...
  it('returns 404 when order not found', async () => {
    ...
    const res = await POST(makeReq({ orderId: 'missing', customer: { name: 'A', phone: '01000000000' } }));
  ...
  it('returns 409 when order status is not pending', async () => {
    ...
    const res = await POST(makeReq({ orderId: 'order-3', customer: { name: 'A', phone: '01000000000' } }));
