...
import { AdminSessionProvider, useAdminSession } from '@/lib/use-admin-session';

interface NavItem {
  href: string;
  label: string;
  icon: string;
  minRole?: string; // minimum role to see this item; default 'staff'
}
...
const navItems: NavItem[] = [
  ...
  { href: '/admin/finance',  label: 'Finance',   icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z', minRole: 'superadmin' },
...
function SidebarNav() {
  ...
  const roleRank: Record<string, number> = { superadmin: 3, admin: 2, staff: 1 };

  const visibleItems = navItems.filter((item) => {
    if (!item.minRole) return true; // visible to all
    return (roleRank[userRole] || 0) >= (roleRank[item.minRole] || 0);
  });
    ...
    <div className="fsa-admin-layout bg-background text-foreground">
      ...
      >
        <Link href="/admin" className="shrink-0 px-[var(--space-md)] py-[var(--space-md)] border-b border-border-light flex items-center gap-[var(--space-sm)] md:flex-col md:items-start md:px-[var(--space-lg)] md:py-[var(--space-lg)]">
          <span className="text-[var(--text-xl)] md:text-[var(--text-2xl)] font-bold tracking-[0.3em] text-foreground font-[family-name:var(--font-playfair)]">7H</span>
      ...
      <div className="fsa-admin-main flex flex-col min-h-screen">
        ...
        <header className="md:hidden flex items-center gap-[var(--space-md)] px-[var(--space-md)] py-[var(--space-sm)] bg-card border-b border-border sticky top-0 z-10">
          ...
          <span className="font-bold tracking-widest text-foreground text-[var(--text-sm)] font-[family-name:var(--font-playfair)]">7H ADMIN</span>
        </header>
